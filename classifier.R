#TotalData = read.csv("/home/asta/Nonparametric-statistics/wine2.csv", header = TRUE)
library(readxl)
...
projected_classifier = function(sample){
  ...
  for (j in 1:length(tt)) {
    ...
    proj_classifier = build_classifier_from_sample(proj_sample)
    success_prob[j] = integrate_classifier(proj_classifier, min(proj_sample), max(proj_sample), 0.5)
...
errors1 = sum(apply(my_data[, -1], 1, empirical_classifier) != my_data[,1])
errors2 = sum(apply(my_data[, -1], 1, proj_classifier) != my_data[,1])

tt = seq(11, 30, 0.1)
angle = 4.7
plot(my_data[,2], my_data[,3], type = "p", col = my_data[,1] + 1)
lines(30+tt * sin(angle), 90 + tt*cos(angle), col = 1, type = "l")
